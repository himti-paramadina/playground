...
class SubmissionsController extends AppController {
	...
	public function history($quizUniqueName) {
		...
		$scoreboardData = $this->Scoreboard->getSingleUserPivotTable($this->Auth->user('id_user'), $quiz, $problems);
			...
			if ($participation) {
				...
				if (strcmp($quiz['Quiz']['identifier'], $this->request->data['quiz_identifier']) == 0) {
					...
					if (strcmp($user['User']['email'], $this->request->data['email']) == 0) {
						...
						if (strtotime(date('Y-m-d H:i:s')) <= strtotime(date($quiz['Quiz']['end_time']))) {
							...
							$response['success'] = false;
							$response['message'] = "Time's up! You can't submit your work any longer for this quiz.";	
						}
					}
					else {
						$response['success'] = false;
